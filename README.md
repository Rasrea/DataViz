...
1. **📥 克隆项目** <br>
   ...
   ```
2. **⚙️ 配置必要信息**
    - 将 `application/prod.yml` 和 `resources/static/js/api.json` 中的IP地址修改为你的服务器IP地址
    - 将 Python 解释器路径修改为你本地的Python路径
    - 使用 `pip install -r requirements.txt` 安装所需的Python依赖包
...
3. **🌍 启动服务器** <br>
   ...
   ```
4. **🌐 访问网站** <br>
打开浏览器，访问 `http://<IP地址>:8080/DataViz.html`。
